...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          '연도별 검진 수치 추이를 보여주는 소형 라인차트입니다. 참고치가 상/하한 range(`kind: "band"`)면 배경 밴드 + 같은 색 점선 경계를, 한쪽 기준(`kind: "line"`)뿐이면 중립색 점선 기준선 하나를 그립니다. `line`에는 `riskBoundary`로 질환의심 기준을 위험색 점선으로 추가할 수 있습니다. y축엔 참고치 값만 눈금으로 표시하고, 각 연도의 실제 데이터 값은 점에 마우스를 올리면 `ChartTooltip`으로 보여줍니다.',
      },
    },
  },
  argTypes: {
...
export const LineReferenceWithRisk: Story = {
  args: {
    values: [124, 118],
    categories: ["'23", "'24"],
    status: 'normal',
  ...
  parameters: {
    docs: {
      description: {
        story:
          '수축기 혈압 추이 — 정상 상한(120, 중립색)과 질환의심 기준(140, 위험색)을 점선 두 개로 함께 표시합니다.',
      },
    },
  },
};
...
export const FourColumns: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: '실제 화면(핵심 지표 추이)처럼 4열로 나란히 배치했을 때의 모습입니다.',
      },
    },
  },
  render: () => (
