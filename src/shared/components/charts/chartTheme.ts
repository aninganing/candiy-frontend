...
// globals.css의 :root(/다크모드 오버라이드)에 정의된 원시 변수명. Chart.js는 canvas에 그리므로
// var(--x) 문자열을 그대로 못 쓰고, 실제 계산된 색상값을 읽어와야 한다.
...
export function useChartColors(): ChartColors {
  ...
  useEffect(() => {
    ...
    // 실제 앱은 prefers-color-scheme만 바뀌지만, Storybook 다크모드 토글처럼
    // data-theme 속성으로 강제 전환하는 경우도 감지해야 캔버스 색상이 stale 되지 않는다.
